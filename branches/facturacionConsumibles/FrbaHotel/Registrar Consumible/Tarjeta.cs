...
namespace FrbaHotel.Registrar_Consumible
{
    public partial class Tarjeta : Form
    {
        ...
        {
            if (DateTime.Compare(DateTime.Today, DateTime.ParseExact(txtEmision.Text, "dd/MM/yyyy", CultureInfo.InvariantCulture)) == -1)
            {
                MessageBox.Show("La fecha de emision no puede ser posterior a hoy");
            } else if (DateTime.Compare(DateTime.ParseExact(txtFechaVence.Text, "dd/MM/yyyy", CultureInfo.InvariantCulture), DateTime.Today) == -1)
            {
